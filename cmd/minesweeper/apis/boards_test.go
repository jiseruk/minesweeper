...
func TestBoard(t *testing.T) {
	...
	runTestsSuite(t, []apiTestCase{
		{"t1 - create a game", "POST", "/api/v1/games/", "/api/v1/games/", `{"width":7, "height":7, "mines":5}`, CreateBoard, http.StatusCreated, path + "/board_tc1.json"},
		{"t2 - select point", "PUT", "/api/v1/games/:id", "/api/v1/games/2", `{"x":1, "y":2}`, SelectPoint, http.StatusOK, path + "/board_tc1.json"},
		{"t3 - Get a game", "GET", "/api/v1/games/:id", "/api/v1/games/2", ``, Get, http.StatusOK, path + "/board_tc1.json"},
	})
}
...
func TestSelectNonMineNeighboorPoint(t *testing.T) {
	...
	res := testAPICall(http.MethodPut, `/api/v1/games/1`, `{"x":1, "y":1}`)
	assert.JSONEq(t, `{"id":1, "width":4, "height":3, "mines":1, "matrix":[[" "," ","1","-"],[" "," ","1","1"],[" "," "," "," "]], "status":"ACTIVE"}`, res.Body.String())
	res = testAPICall(http.MethodPut, `/api/v1/games/1`, `{"x":4, "y":1}`)
...
func TestSelectMinePointAndWin(t *testing.T) {
	...
	res := testAPICall(http.MethodPut, `/api/v1/games/1`, `{"x":4, "y":1, "mine":true}`)
...
func TestSelectAlreadySelectedPoint(t *testing.T) {
	...
	res := testAPICall(http.MethodPut, `/api/v1/games/1`, `{"x":2, "y":1}`)
